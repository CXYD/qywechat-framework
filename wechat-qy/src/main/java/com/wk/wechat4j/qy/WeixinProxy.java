...
import com.wk.wechat4j.base.model.MediaCounter;
import com.wk.wechat4j.base.model.MediaDownloadResult;
import com.wk.wechat4j.base.model.MediaItem;
import com.wk.wechat4j.base.model.MediaRecord;
import com.wk.wechat4j.base.model.MediaUploadResult;
import com.wk.wechat4j.base.model.Pageable;
import com.wk.wechat4j.base.model.WeixinAccount;
import com.wk.wechat4j.qy.api.AgentApi;
import com.wk.wechat4j.qy.api.BatchApi;
import com.wk.wechat4j.qy.api.ChatApi;
import com.wk.wechat4j.qy.api.HelperApi;
import com.wk.wechat4j.qy.api.MediaApi;
import com.wk.wechat4j.qy.api.MenuApi;
import com.wk.wechat4j.qy.api.NotifyApi;
import com.wk.wechat4j.qy.api.PartyApi;
import com.wk.wechat4j.qy.api.TagApi;
import com.wk.wechat4j.qy.api.UserApi;
...
import com.wk.wechat4j.qy.suite.SuitePerCodeHolder;
import com.wk.wechat4j.qy.suite.WeixinTokenSuiteCreator;
import com.wk.wechat4j.qy.token.WeixinTicketCreator;
import com.wk.wechat4j.qy.token.WeixinTokenCreator;
...
public class WeixinProxy {
	...
	/**
	 * 默认使用文件方式保存token、使用weixin4j.properties配置的账号信�?
	 */
	public WeixinProxy() {
	 ...
	 * @param settings
	 *            微信配置信息
	 * @see com.wk.wechat4j.base.util.Weixin4jSettings
	 */
	 ...
	 * @param perCodeHolder
	 *            第三方套件永久授权码
	 *            {@link com.wk.wechat4j.qy.api.SuiteApi#getPerCodeHolder(String)}
	 * @param suiteTokenHolder
	 *            第三方套件凭证token
	 *            {@link com.wk.wechat4j.qy.api.SuiteApi#getTokenSuiteHolder(String)}
	...
	/**
	 * 注意：TokenCreator �?�? <font
	 * color="red">WeixinTokenCreator或WeixinTokenSuiteCreator</font>
	 * 
	 * @see com.wk.wechat4j.qy.token.WeixinTokenCreator
	 * @param tokenHolder
	 */
	...
	/**
	 * 发�?�客服消�?(�?要管理员对应用有使用权限，对收件人touser、toparty、totag有查看权限，否则本次调用失败)
	 * <p>
	 * 1�? 发�?�人员列表存在错误的userid：执行发送，�?发�?�需注意返回结果说明</br>
	 * 2）发送人员不在�?�讯录权限范围内：不执行发�?�任务，返回首个出错的userid</br>
	 * 3）发送人员不在应用可见范围内：不执行发�?�任务，返回首个出错的userid</br>
	 * </p>
	 *
	 * @param message
	 *            客服消息对象
	 * @return 如果对应用或收件人�?�部门�?�标签任何一个无权限，则本次发�?�失败；如果收件人�?�部门或标签不存在，发�?�仍然执行，但返回无效的部分
	 *         </br> { "errcode": 0, "errmsg": "ok", "invaliduser": "UserID1",
	 *         "invalidparty":"PartyID1", "invalidtag":"TagID1" }
	 * @throws WeixinException
	 * @see com.wk.wechat4j.qy.api.NotifyApi
	 * @see <a href=
	...
	/**
	 * 自定义菜�?(管理员须拥有应用的管理权�? 并且应用必须设置在回调模�?)
	 *
	 * @param buttons
	 *            菜单列表
	 * @param agentid
	 *            应用ID
	 * @throws WeixinException
	 * @see com.wk.wechat4j.qy.api.MenuApi
	 * @see <a href=
	...
	/**
	 * 查询菜单(管理员须拥有应用的管理权�? 并且应用必须设置在回调模式�??)
	 *
	 * @param agentid
	 *            应用ID
	 * @return 菜单集合
	...
	/**
	 * 删除菜单(管理员须拥有应用的管理权�? 并且应用必须设置在回调模�?)
	 *
	 * @param agentid
	 *            应用ID
	 * @throws WeixinException
	 ...
	 * @return 图片url
	 * @see com.wk.wechat4j.qy.api.MediaApi
	 * @throws WeixinException
	 */
	 ...
	 *
	 * @param agentid
	 *            企业应用ID(<font color="red">大于0时视为上传永久媒体文�?</font>)
	 * @param is
	 *            媒体数据�?
	 * @param fileName
	 *            文件�?
	 * @return 上传到微信服务器返回的媒体标�?
	 * @see com.wk.wechat4j.qy.api.MediaApi
	 ...
	 *
	 * @param agentid
	 *            企业应用Id(<font color="red">大于0时视为获取永久媒体文�?</font>)
	 * @param mediaId
	 *            媒体ID
	 * @return 媒体下载结果
	 ...
	 * 上传永久图文素材
	 * <p>
	 * 、新增的永久素材也可以在公众平台官网素材管理模块中看�?,永久素材的数量是有上限的，请谨慎新增。图文消息素材和图片素材的上限为5000�?
	 * 其他类型�?1000
	 * </P>
	 *
	 * @param agentid
	 ...
	 *            企业应用ID
	 * @param mediaType
	 *            素材的类型，图片（image）�?�视频（video）�?�语�? （voice）�?�图文（news）�?�文件（file�?
	 * @param pageable
	 *            分页数据
	 * @return 媒体素材的记录对�?
	 * @throws WeixinException
	 ...
	 *      创建部门说明</a>
	 * @see com.wk.wechat4j.qy.api.PartyApi
	 * @return 部门ID
	 * @throws WeixinException
	 */
	 ...
	 *            部门列表
	 * @see com.wk.wechat4j.qy.api.MediaApi
	 * @see com.wk.wechat4j.qy.api.BatchApi
	 ...
	 *      创建成员说明</a>
	 * @see com.wk.wechat4j.qy.api.UserApi
	 * @return 处理结果
	 * @throws WeixinException
	 */
	...
	/**
	 * code获取userid(管理员须拥有agent的使用权限；agentid必须和跳转链接时�?在的企业应用ID相同�?)
	 *
	 * @param code
	 *            通过员工授权获取到的code，每次员工授权带上的code将不�?样，code只能使用�?次，5分钟未被使用自动过期
	 * @see com.wk.wechat4j.qy.model.User
	 * @see com.wk.wechat4j.qy.api.UserApi
	 * @return 成员对象
	 ...
	 * @param code
	 *            通过员工授权获取到的code，每次员工授权带上的code将不�?样，code只能使用�?次，5分钟未被使用自动过期
	 * @return 两个元素的数�? <font color="red">第一个元素为userId或�?�openId
	 *         第二个元素为deviceId</font>
	 * @see com.wk.wechat4j.qy.api.UserApi
	 * @see <a href=
	 ...
	 *            成员ID
	 * @param tips
	 *            推�?�到微信上的提示语（只有认证号可以使用）。当使用微信推�?�时，该字段默认为�?�请关注XXX企业号�?�，邮件�?请时，该字段无效�?
	 * @see com.wk.wechat4j.qy.api.UserApi
	 * @return 调用结果
	 * @see <a href=
	 ...
	 *      创建标签说明</a>
	 * @see com.wk.wechat4j.qy.api.TagApi
	 * @return 标签ID
	 * @throws WeixinException
	 */
	 ...
	 *      获取标签成员说明</a>
	 * @see com.wk.wechat4j.qy.api.TagApi
	 * @return 成员列表<font color="red">Contacts#getUsers</font>和部门列�?<font
	 ...
	 * <font color="red">若部分userid非法，则在text中体�?</font>
	 *
	 * @param tagId
	 *            标签ID
	 * @param userIds
	 ...
	 *
	 * @return IP地址
	 * @see com.wk.wechat4j.qy.api.HelperApi
	 * @see <a href=
	 ...
	 * @see com.wk.wechat4j.qy.api.AgentApi
	 * @see <a href=
	...
	/**
	 * 批量更新成员,本接口以userid为主键，增量更新企业号�?�讯录成员�??
	 * <p>
	 * 1.模板中的部门�?填写部门ID，多个部门用分号分隔，部门ID必须为数�?</br>
	 * 2.文件中存在�?��?�讯录中也存在的成员，更新成员在文件中指定的字段�? </br> 3.文件中存在�?��?�讯录中不存在的成员，执行添加操�?</br>
	 * 4.通讯录中存在、文件中不存在的成员，保持不�?</br>
	 * </p>
	 *
	 * @param mediaId
	...
	/**
	 * 批量覆盖成员,本接口以userid为主键，全量覆盖企业号�?�讯录成员，任务完成后企业号通讯录成员与提交的文件完全保持一致�??
	 * <p>
	 * 1.模板中的部门�?填写部门ID，多个部门用分号分隔，部门ID必须为数�?</br> 2.文件中存在�?��?�讯录中也存在的成员，完全以文件为准</br>
	 * 3.文件中存在�?��?�讯录中不存在的成员，执行添加操�?</br>
	 * 4.通讯录中存在、文件中不存在的成员，执行删除操作�?�出于安全�?�虑，如果需要删除的成员多于50人，
	 * 且多于现有人数的20%以上，系统将中止导入并返回相应的错误�?
	 * </p>
	 *
	 * @param mediaId
	 ...
	 * <p>
	 * 1.文件中存在�?��?�讯录中也存在的部门，执行修改操�?</br> 2.文件中存在�?��?�讯录中不存在的部门，执行添加操�?</br>
	 * 3.文件中不存在、�?�讯录中存在的部门，当部门为空时，执行删除操�?</br>
	 * 4.CSV文件中，部门名称、部门ID、父部门ID为必填字段，部门ID必须为数字；排序为可选字段，置空或填0不修改排�?
	 * </p>
	 *
	 * @param mediaId
	 ...
	 * �?要自行将企业号的userid转成openid�? 在使用微信红包功能时，需要将应用id和userid转成appid和openid才能使用�?
	 *
	 * @param userid
	 *            企业号内的成员id 必填
	 * @param agentid
	 *            �?要发送红包的应用ID，若只是使用微信支付和企业转账，则无�?该参�? 传入0或负数则忽略
	 * @return 结果数组 第一个元素为对应的openid 第二个元素则为应用的appid(如果�?)
	 * @throws WeixinException
	 * @see com.wk.wechat4j.qy.api.UserApi
	 * @see <a href=
	 ...
	 * �?发�?�需要知道某个结果事件的openid对应企业号内成员的信息时，可以�?�过调用该接口进行转换查询�??
	 *
	 * @param openid
	...
	/**
	 * 创建会话 <font color="red">如果会话id为空,程序会自动生成一个唯�?ID</font>
	 *
	 * @param chatInfo
	 *            会话信息
	 * @return 会话ID
	 * @see com.wk.wechat4j.qy.api.ChatApi
	...
	/**
	 * 设置成员接收到的消息是否提醒。主要场景是用于对接企业im的在线状态，如成员处于在线状态时，可以设置该成员的消息免打扰。当成员离线时，关闭免打扰状�?
	 * ，对微信端进行提醒�??
	 *
	 * @param chatMutes
	 *            提醒参数
